...
import {Prisma, OperDagRapport, RefLid} from "@prisma/client";
...
export class DagRapportenService extends IHeliosService
{
   ...
   async AddObject(data: Prisma.OperDagRapportCreateInput, actorId: number): Promise<OperDagRapport>
   {
      ...
      this.logger.verbose(`DagRapportenService.AddObject() => ${safeStringify(obj)}`);
      return obj;
   }

   async UpdateObject(id: number, data: Prisma.OperDagRapportUpdateInput, actorId: number): Promise<OperDagRapport>
   {
      ...
      this.logger.verbose(`DagRapportenService.UpdateObject() => ${safeStringify(obj)}`);
      return obj;
   }
